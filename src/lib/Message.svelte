<script>
	...
	$: {
		...
		positionStore = tweened(0, {easing, duration: user ? 100 : 3000})
...
<p class="{user ? "text-neutral-500" : "text-neutral-100"} text-wrap whitespace-pre py-3">&gt {message.substr(0, position)}</p>